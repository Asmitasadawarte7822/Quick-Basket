...
if (!empty($search)) {
    $search_param = "%$search%";
    $where .= " AND (store_name LIKE ? OR email LIKE ? OR phone LIKE ?)";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
    $types .= "sss";
...
// Count total sellers
$count_sql = "SELECT COUNT(*) AS total FROM sellers $where";
...
$total_sellers = $total_row['total'];
$total_pages = ceil($total_sellers / $limit);

// Fetch sellers
$sql = "SELECT id, store_name, email, phone, status, created_at FROM sellers $where ORDER BY store_name LIMIT ? OFFSET ?";
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        <div class="admin-topbar">
            <h1><i class="fa-solid fa-store"></i> Manage Sellers</h1>
            <div class="right-actions">
                <a href="add-seller.php" class="btn-add">
                    <i class="fa-solid fa-plus"></i> Add Seller
                </a>
                <form method="GET" class="search-form">
                    <input type="text" name="search" placeholder="Search by store name, email or phone..." value="<?php echo htmlspecialchars($search); ?>">
                    <button type="submit"><i class="fa-solid fa-search"></i></button>
                    <?php if (!empty($search)): ?>
                        <a href="sellers.php" class="btn-clear">Clear</a>
                    <?php endif; ?>
                </form>
            </div>
        </div>
                    ...
                    <?php if ($result && mysqli_num_rows($result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="edit-seller.php?id=<?php echo $row['id']; ?>" class="action-btn edit" title="Edit">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <a href="delete-seller.php?id=<?php echo $row['id']; ?>" class="action-btn delete" onclick="return confirm('Delete this seller?')" title="Delete">
                                        <i class="fa-regular fa-trash-can"></i>
                                    </a>
                                    <a href="../category-products.php?slug=<?php echo $row['store_name']; ?>" class="action-btn view" target="_blank" title="View Products">
