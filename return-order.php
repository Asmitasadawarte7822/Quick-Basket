...
$check_sql = "SELECT id, status FROM orders WHERE id = ? AND user_id = ? AND status = 'delivered'";
...
if (mysqli_num_rows($result) > 0) {
    ...
    if (mysqli_num_rows($table_check) > 0) {
        ...
        $note = 'Order returned by user.';
        $history_stmt = mysqli_prepare($conn, $history_sql);
        mysqli_stmt_bind_param($history_stmt, "iss", $order_id, 'refunded', $note);
    ...
    $_SESSION['message'] = 'Cannot return this order.';
}
